...
# This function runs a basic port scan using nmap and saves the output to a file in the report directory.
function portScan_(){
    echo
    echo "${BCYAN}[+]${RESET} Performing Port Scan..."
    nmap -T4 -p- $TARGET -oN $REPORT_DIR/port_scan.txt
    sleep 1
    echo
    echo "${ON_GREEN}${WHITE}[!] Port Scan for $TARGET done.${RESET}"
    }

##############################################################################################

# This function performs subdomain enumeration using Sublist3r and saves the output to a file in the report directory.
function sublist3rScan_(){
    echo
    echo "${BCYAN}[+]${RESET} Performing Subdomain Enumeration with Sublist3r..."
    python3 /path/to/Sublist3r/sublist3r.py -d $TARGET -o $REPORT_DIR/sublist3r_scan.txt
    sleep 1
    echo
    echo "${ON_GREEN}${WHITE}[!] Sublist3r scan for $TARGET done.${RESET}"
}

##############################################################################################

# This function runs a directory and file enumeration using gobuster and saves the output to a file in the report directory.
function gobusterScan_(){
    echo
    echo "${BCYAN}[+]${RESET} Performing Directory and File Enumeration with Gobuster..."
    gobuster dir -u $TARGET -w /usr/share/wordlists/dirb/common.txt -o $REPORT_DIR/gobuster_scan.txt
    sleep 1
    echo
    echo "${ON_GREEN}${WHITE}[!] Gobuster scan for $TARGET done.${RESET}"
}
